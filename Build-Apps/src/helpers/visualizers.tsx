...
export interface VisualizerType {
  ...
  dataType: "time" | "frequency";
  settingsConfig?: Record<string, { type: 'range' | 'color' | 'number' | 'select', min?: number, max?: number, step?: number, options?: string[], default: any }>;
}
  ...
  rainbowSpiral: {
    ...
    draw: function (
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        const angle =
          (i * 2 * Math.PI) / bufferLength +
          Date.now() / (1000 / rotationSpeed);
  ...
  nebulaSpectrogram: {
    ...
    draw: function (
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        const x =
          canvas.width / 2 + Math.cos(angle) * (amplitude * radiusScale);
        const y =
          canvas.height / 2 + Math.sin(angle) * (amplitude * radiusScale);
      ...
      for (let i = 0; i < buildingCount; i++) {
        ...
        for (let row = 0; row < rows; row++) {
          for (let col = 0; col < cols; col++) {
            ...
            ctx.fillStyle = windowColor.replace(
              "{brightness}",
              `${brightness}`,
            );
            ctx.fillRect(
    ...
    draw: function (
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        if (i === 0) ctx.moveTo(x, y);
          ...
          const cp1y =
            y + Math.sin(Date.now() / 1000 + i / 20) * curveAmplitude;
