...
export function calculateGaplessOffsets(song?: Song | null): GaplessOffsets {
  ...
  const startSeconds = typeof delaySamples === "number" && Number.isFinite(delaySamples) && delaySamples > 0
    ? delaySamples / sampleRate
    : 0;
  const endSeconds = typeof paddingSamples === "number" && Number.isFinite(paddingSamples) && paddingSamples > 0
