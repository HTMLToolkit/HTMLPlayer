...
export const useMusicPlayer = () => {
  ...
  const cacheSong = async (song: Song) => {
    ...
    try {
      ...
      if (song.hasStoredAudio) {
        ...
        if (audioData) {
          const url = URL.createObjectURL(new Blob([audioData.fileData], { type: audioData.mimeType }));
        ...
        if (persistedLibrary) {
          ...
          const updatedLibrary = {
            ...
            playlists: validLibrary.playlists.some(p => p.id === 'all-songs')
              ? validLibrary.playlists.map(p => p.id === 'all-songs' ? { ...p, songs: prepareSongsForPlaylist(validLibrary.songs) } : p)
          ...
          if (persistedSettings?.lastPlayedSongId) {
            songToPlay = updatedLibrary.songs.find(s => s.id === persistedSettings.lastPlayedSongId) || null;
          }
          if (persistedSettings?.lastPlayedPlaylistId) {
            playlistToSet = updatedLibrary.playlists.find(p => p.id === persistedSettings.lastPlayedPlaylistId) || null;
          }
  ...
  const playSong = useCallback(
    async (song: Song, playlist?: Playlist) => {
      ...
      const songToPlay = song.hasStoredAudio ? {
        ...song,
        url: '' // Clear URL to force loading from IndexedDB
      } : song;
    ...
    [setupAudioContext, cacheSong, getCachedSong, updateSongCache]
  );
  ...
  const addSong = useCallback(async (song: Song) => {
    ...
    try {
      ...
      setLibrary((prev) => ({
        ...prev,
        songs: prev.songs.map(s => s.id === processedSong.id ? processedSong : s)
      }));
    } catch (error) {
