...
export const PlaylistComponent = ({ komorebi }: PlaylistProps) => {
  ...
  useEffect(() => {
    ...
    const updatePlaylistImages = async () => {
      ...
      const allPlaylists = findAllPlaylists(libraryState.playlists);
  ...
  const handleAllSongsClick = useCallback(() => {
    const existingAllSongs = libraryState.playlists.find(
      (p) => p.id === "all-songs",
    );
    if (
      existingAllSongs &&
      ...
      komorebi.playSong(existingAllSongs.songs[0], existingAllSongs);
      goToSongs();
    } else if (libraryState.songs.length > 0) {
      const allSongsPlaylist: Playlist = {
        id: "all-songs",
        name: t("allSongs"),
        songs: libraryState.songs,
      };
      komorebi.playSong(allSongsPlaylist.songs[0], allSongsPlaylist);
      goToSongs();
    }
  }, [libraryState.playlists, libraryState.songs, komorebi, goToSongs, t]);
  ...
  const handleDeleteConfirm = useCallback(() => {
    ...
    library.removePlaylist(playlistToDelete.id);
    toast.success(
      t("playlist.playlistDeleted", { name: playlistToDelete.name }),
    );
    setIsDeleteDialogOpen(false);
    setPlaylistToDelete(null);
  }, [playlistToDelete, removePlaylistFn]);
  ...
  const handleMoveToFolder = useCallback(
    (item: Playlist | PlaylistFolder) => {
      ...
      const allFolders = getAllFolders(libraryState.playlists);
