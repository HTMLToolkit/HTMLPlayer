...
import { debounce, getValidTempo } from "../helpers/musicPlayerUtils";
import { createCacheManager } from "../helpers/cacheManager";
import { getSmartShuffledSong, createNextSongManager } from "../helpers/shuffleManager";
...
export const useMusicPlayer = () => {
  ...
  const nextSongManager = createNextSongManager(nextSongCacheRef, nextSongCacheValidRef);
  ...
  const { getAndCacheNextSong, invalidateNextSongCache } = nextSongManager;
      ...
      try {
        ...
        if (persistedLibrary) {
          ...
          const updatedLibrary = {
            ...validLibrary,
            playlists: validLibrary.playlists.some((p: any) => p.id === "all-songs")
            ...
            if (persistedSettings?.lastPlayedPlaylistId) {
              playlistToSet =
                updatedLibrary.playlists.find(
                  (p: Playlist) => p.id === persistedSettings.lastPlayedPlaylistId
                ) || null;
            }
          }
  ...
  useEffect(() => {
    ...
    if (
      ...
      (playerState.shuffle && previousSettings.smartShuffle !== settings.smartShuffle)
    ) {
      console.log('Smart shuffle setting changed, invalidating next song cache');
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log('Player state affecting next song changed, invalidating cache');
      invalidateNextSongCache();
    }
  }, [playerState.shuffle, playerState.currentPlaylist?.id, playerState.repeat, invalidateNextSongCache]);
  ...
  useEffect(() => {
    ...
    if (settingsRef.current.smartShuffle && playerStateRef.current.shuffle && recentlyPlayed.length > 0) {
      invalidateNextSongCache();
    }
  ...
  const playSong = useCallback(
    async (song: Song, playlist?: Playlist) => {
      ...
      if (audioRef.current) {
        ...
        if (playlist) {
          updateSongCache(song, playlist, playerStateRef, settingsRef, recentlyPlayed, getSmartShuffledSong);
        }
    ...
    const handleTimeUpdate = () => {
      ...
      if (
        ...
        if (timeRemaining <= preloadTime && timeRemaining > preloadTime - 0.5) {
          preloadNextSong(
            () => getNextSongForCrossfade(getAndCacheNextSong),
            cacheSong,
            getCachedSong,
            settingsRef
          );
        ...
        if (timeRemaining <= crossfadeDuration && timeRemaining > crossfadeDuration - 0.5) {
          ...
          startCrossfadeTransition(
            playNextRef,
            () => getNextSongForCrossfade(getAndCacheNextSong),
            (nextSong: Song) => prepareNextSongForCrossfade(nextSong, cacheSong, getCachedSong, settingsRef),
            (nextSong: Song) => updatePlayerStateAfterCrossfade(nextSong, playerStateRef, recentlyPlayed, setRecentlyPlayed, setPlayerState),
            settingsRef
          );
        }
      }
  ...
  const playNext = useCallback(() => {
    ...
    let nextSong = getAndCacheNextSong(playerStateRef, settingsRef, recentlyPlayedRef);
  ...
  }, [playerState, playSong, settings.smartShuffle, recentlyPlayed, setRecentlyPlayed]);
