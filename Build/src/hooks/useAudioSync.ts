...
export const useAudioSync = ({
  ...
  const isCrossfading =
    (musicPlayerHook as any).crossfadeManagerRef?.current?.isCrossfading?.() ||
    false;
  ...
  const handleVolumeEvent = useCallback(
    (event: any) => {
      if (
        event.source !== source &&
        event.payload?.volume !== playerState.volume
      ) {
        console.log(
          `[${source}] Received volume event from ${event.source}:`,
  ...
  const handleSeekEvent = useCallback(
    (event: any) => {
      if (
        event.source !== source &&
        Math.abs(event.payload?.time - playerState.currentTime) > 1
      ) {
        console.log(
          `[${source}] Received seek event from ${event.source}:`,
          event.payload.time,
        );
